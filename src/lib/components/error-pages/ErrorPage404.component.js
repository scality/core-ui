...
const translations = {
  en: {
    ...
    should_do: "You might check the url, or contact your admin if the error remains.",
    return_home: "Return Home",
  },
  fr: {
    not_exist: "Erreur: cette page n'existe pas",
    error_desc: "Il se peut que ce soit une mauvaise adresse ou que la page ait été déplacée.",
    should_do: "Vous pouvez vérifier l'url, ou contacter votre administrateur si l'erreur persiste.",
...
function ErrorPage404({
  ...
  return (
    <ErrorPageContainer className="sc-error-page404" { ...rest }>
      ...
      <Row>
        <Description>
          <DescriptionContent>{ translations[locale].error_desc }</DescriptionContent>
          <DescriptionContent>{ translations[locale].should_do }</DescriptionContent>
        </Description>
      </Row>
      <Button
