...
const options = Array.from(new Array(10), (_, index) => (<Select.Option value={`Option${index}`} >{`Label-option ${index}`}</Select.Option>))
...
export const DifferentSizes = {
  render:(args) => (
    <SelectWrapper>
      {sizes.map(size => <Select size={size} {...args} ></Select>)}
    </SelectWrapper>
  )
}
