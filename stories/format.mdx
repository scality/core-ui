...
This does not cover table columns.

## Separating two values

Two values shown together on one line are separated by " - ", a hyphen with a space on each side.
Never a middle dot.

## Value and total

A value shown against its total is written `8 GiB /47 GiB`, with a space before the slash and none after.
The value takes `textPrimary`, the slash and the total take `textSecondary`.
