...
When the action linked to a button is not possible, the button should be visually disabled rather than
removed from the interface for consistent user interaction.
A hovering tooltip must be provided to explain the reason behind the button's disabled condition.
