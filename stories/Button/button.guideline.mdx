...
| Destructive action, or one that interrupts something running (delete, reset, stop, disable, revoke) | `danger` |
...
Used for actions that destroy data or interrupt something that is running: delete, reset, stop, disable, revoke. An action does not have to be irreversible: stopping a running service can be undone and still takes `danger`. When the main action is destructive, use `danger` instead of `primary`. Never use them alongside each other. In a destructive confirmation context, `danger` takes the primary position (right side of the group).
